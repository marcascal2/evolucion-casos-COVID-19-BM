...
# AÃ±adir "provincia", "iso" (CCAA) y "ccaa"
casos <- casos %>% left_join(provincias, by="provincia_iso")

# -------------------------------------------------------------------------------------------------------------------------
# TRANSFORMACIÓN DE VALORES DE CADENAS EN VALORES NUMÉRICOS PARA LA APLICACIÓN DE BALLMAPPER
...
data <- subset(casos_ccaa, fecha>="2020-03-01")
data <- subset(data, fecha<="2020-04-30")

## ELIMINAMOS FILAS QUE NO APORTAN DATOS RELEVANTES
data <- subset(data, sexo==2)
data <- subset(data, edad!=-1)
data <- subset(data, iso!='NA')

## CONVERTIMOS LA ULTIMA COLUMA CATEGÓRICA EN NUMÉRICA
